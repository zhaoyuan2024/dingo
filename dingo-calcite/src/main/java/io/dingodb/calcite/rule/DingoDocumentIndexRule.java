...
//        RelNode relNode = null;
    ...
    public static RelNode getDingoGetDocumentByKeyword(RexNode condition, LogicalDingoDocument document, boolean forJoin) {
...
//        String targetDocument = getTargetKeyword(document.getOperands());
        ...
        // plan A: DingoTableScan pushdown ->document get distance and sorted bydocumentIds
        // plan B: DingoTableScan pushdown ->text search bydocumentIds
        // 2.document search operator pushdown
        // plan C:table_data pushdown operator ->document get distance and sorted bydocumentIds
        // plan D: table_data pushdown operator ->text search bydocumentIds
        // 3. scalar index ->text search bydocumentIds
        // scalar operator ->text search bydocumentIds
        // scalar operator ->text get distance and sorted bydocumentIds
    ...
    private static RelNode prePrimaryOrScalarPlan(
        ...
        DocumentStreamConvertor documentStreamConvertor = new DocumentStreamConvertor(
            ...
            document.getIndexTable(),
            false);
        DingoGetDocumentByKeyWord dingoGetDocumentByKeyWord = new DingoGetDocumentByKeyWord(
            document.getCluster(),
            traitSet,
            documentStreamConvertor,
            condition,
